...
class GraphCNN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim, final_dropout, learn_eps, graph_pooling_type, neighbor_pooling_type, device):
        ...
        # for Neural Pooling Method 2
        self.total_latent_dim = input_dim + hidden_dim * (num_layers - 1)
        self.inter_dim = 32
        self.inter_inter_dim = 64
        self.dense_dim = self.inter_dim * self.inter_dim
        self.fcl1 = nn.Linear(self.total_latent_dim, self.inter_inter_dim, bias=False)
        self.fcl2 = nn.Linear(self.inter_inter_dim, self.inter_dim, bias=False)
    ...
    def forward(self, batch_graph):
        ...
        for g_i in range(len(graph_sizes)):
            ...
            cur_node_embeddings = self.fcl1(cur_node_embeddings)
            cur_node_embeddings = self.fcl2(cur_node_embeddings)
