...
class GraphCNN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim, final_dropout, learn_eps, graph_pooling_type, neighbor_pooling_type, device):
        ...
        self.attend = nn.Linear(100, 1)
        self.linear1 = nn.Linear(100, output_dim)
    ...
    def forward(self, batch_graph):
        ...
        batch_graphs = torch.zeros(len(graph_sizes), 100).to(self.device)
        ...
        for g_i in range(len(graph_sizes)):
            ...
            cur_node_embeddings -= torch.mean(cur_node_embeddings,dim=0)
            cov = torch.matmul(cur_node_embeddings.transpose(0,1),cur_node_embeddings)/cur_node_embeddings.shape[0]
            cov = cov.detach().cpu().numpy()
            #X = cur_node_embeddings.detach().cpu().numpy()
            U,S,V = np.linalg.svd(cov)
            U = torch.from_numpy(U).cuda(device=self.device)
            cur_node_embeddings = torch.matmul(cur_node_embeddings,U[:,:100])
            ...
            #print(cur_graph_embeddings.shape)
            batch_graphs[g_i] = cur_graph_embeddings.view(100)
